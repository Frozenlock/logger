...
;;;;;;;;;;;;;;;;;;;;;
(defn not=*
  "Temporary patch for Clojure regexp equality bug."
  [& args]
  (->> (map #(if (= (type %) java.util.regex.Pattern)
               (str %) %) args)
       (apply not=)))
  ...
  "Get the local configs, fetch them from the server, and merge
   ...
   (try (let [local-configs (get-configs-only)
              remote-configs (-> (project-configs-from-server local-configs)
                                 remove-nil-in-maps)]
          (when (seq (map-difference remote-configs (dissoc local-configs :logger-password)))
